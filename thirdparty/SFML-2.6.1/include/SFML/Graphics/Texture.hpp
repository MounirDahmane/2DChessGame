...
private:
    ...
    Vector2u
        m_actualSize; //!< Actual texture size (can be greater than public size because of padding)
...
///
///     // update the texture
///     sf::Uint8* pixels = ...; // get a fresh chunk of pixels (the next frame of a movie, for
///     example) texture.update(pixels);
///
///     // draw it
///     window.draw(sprite);
///
